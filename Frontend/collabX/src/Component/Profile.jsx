...
function Profile() {
  ...
  async function getProfile() {
    try {
      const response = await axios.get(`http://localhost:8090/profiles/${user.username}`);
  ...
  const handleSave = async () => {
    try {
      ...
      const response = await axios.patch(`http://localhost:8090/profiles/${user.username}`, updatedProfile);
        ...
        <div className="flex flex-col items-center sm:items-start gap-8 sm:w-1/3">
          <div className="w-full flex justify-between items-center">
            <h1 className="text-2xl font-semibold text-teal-600">
              {isEditing ? (
                <input
                  ...
                  className="bg-transparent border-b-2 border-teal-600 outline-none"
                />
              ) : (
            ...
            <div className="flex gap-4">
              <Link>
                <FontAwesomeIcon icon={faLinkedin} className="text-teal-600 hover:text-teal-800 transition-colors" />
              </Link>
              <Link>
                <FontAwesomeIcon icon={faGithub} className="text-teal-600 hover:text-teal-800 transition-colors" />
              </Link>
              <Link>
                <FontAwesomeIcon icon={faFacebook} className="text-teal-600 hover:text-teal-800 transition-colors" />
              </Link>
              <Link>
                <FontAwesomeIcon icon={faTwitter} className="text-teal-600 hover:text-teal-800 transition-colors" />
              </Link>
              <Link>
                <FontAwesomeIcon icon={faDribbble} className="text-teal-600 hover:text-teal-800 transition-colors" />
              </Link>
            </div>
          </div>
              ...
              <p className="text-gray-700 w-2/3">
                {isEditing ? (
                  <div className="flex items-center">
                    <span className="text-teal-600 mr-2">Can't edit username</span>
                    <input
                      type="text"
                      value={userDetails.username}
                      onChange={(e) => handleChange("username", e.target.value)} // Fix this to update the username
              ...
              <p className="text-gray-700 w-2/3">
                {isEditing ? (
                  <input
                    ...
                    className="bg-transparent border-b-2 border-teal-600 outline-none py-1"
                  />
                ) : (
                  usersData.email
                )}
            ...
            <div className="flex justify-between items-center space-y-1">
              ...
              <p className="text-gray-700 w-2/3">
                {isEditing ? (
                  ...
                  usersData.birthday ? usersData.birthday.substring(0, 10) : "N/A"
                )}
              </p>
            </div>
...
export default Profile;
  ...
  //   async function getProfile() {
  //     try {
  //       const response = await axios.get(`http://localhost:8090/users/${user.username}`);
