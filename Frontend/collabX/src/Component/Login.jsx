...
function Login() {
  ...
  async function getData() {
    try {
      const response = await axios.get(
        "https://hackathonfeb2025users-default-rtdb.asia-southeast1.firebasedatabase.app/.json"
      );
      let arr = [];
      let obj = {};
  ...
  function loginCheck(e) {
    ...
    for (let i = 0; i < usersData.length; i++) {
      if (
        usersData[i].username === userCheck.username &&
        usersData[i].password === userCheck.password
      ) {
